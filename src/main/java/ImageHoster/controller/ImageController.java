...
public class ImageController {
    ...
    /*    This request mapping has been changed to accept the image id from the html page.
    The method internally now uses the image id, instead of image title, for retrieval of the image from the DB*/
    ...
    /*This method first calls the business logic ( compareUserAndOwner() ) to check if the logged in user is the same as owner of image.
    If the logged in user is the same as owner of the image, the image is allowed to be edited, otherwise an error is thrown*/
    @RequestMapping(value = "/editImage")
    public String editImage(@RequestParam("imageId") Integer imageId, HttpSession session, Model model) {

        Image image = imageService.getImage(imageId);
        User logggedInUser = (User) session.getAttribute("loggeduser");

        if(imageService.compareUserAndOwner(logggedInUser,image)){
    ...
    public String editImageSubmit(@RequestParam("file") MultipartFile file, @RequestParam("imageId") Integer imageId, @RequestParam("tags") String tags, Image updatedImage, HttpSession session) throws IOException {
        ...
        //The return statement has been changed as per the request mapping of the handler method, to include Id in URL
    ...
    /*This method first calls the business logic ( compareUserAndOwner() ) to check if the logged in user is the same as owner of image.
    If the logged in user is the same as owner of the image, the image is allowed to be deleted, otherwise an error is thrown*/
